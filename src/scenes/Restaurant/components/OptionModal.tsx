...
const OptionModal: React.FC<Props> = ({
  ...
  const { addProduct, shop } = cartModal();

  const [sizeOption, setSizeOption] = useState<string | null>(
    options.sizes?.map((size) => size.name)[0] || null
  );
      ...
      <div className="fixed left-[50%] top-[50%] min-w-[350px]  bg-gray-100  shadow rounded translate-x-[-50%] translate-y-[-50%] z-50 grid grid-cols-1 lg:grid-cols-2 p-5">
        ...
        <div className="p-5">
          ...
          {options.sizes && options.sizes.length > 0 && (
            <>
              {" "}
              <div className="bg-gray-200 p-5 my-2">
